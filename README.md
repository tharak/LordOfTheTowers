...
**Follow-up addition:** `AEnemyPawn::SetAlertVisual(bool)` (called from `AEnemyAIController::OnTargetPerceptionUpdated`) drives a runtime dynamic material instance to tint the plane solid red while the enemy has a target, off otherwise — a visible "found you" indicator. Uses `TextureColour`/`ColourMask` on `MI_PlaneEnemy` (a Lerp blend that fully overrides base color) — **not** `EmissiveColour`/`EmissiveStrength`, which is silently gated to zero by `TextureORME`'s alpha channel on this mesh and never shows regardless of value. Debounced with a 1.5s `SetTimerByFunctionName`/`ClearTimerByFunctionName` pair (target `ApplyAlertOff`), since AI sight perception re-fires every few seconds even for a stationary, continuously-visible target — without the debounce the tint flickers too fast to notice.
